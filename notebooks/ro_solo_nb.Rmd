...
districts %>%
  ...
  geom_point(color = "#333333", fill = "lightsteelblue4", pch = 21, size = 2) +
  geom_vline(xintercept = -1.5, size = 1.5, color = "#333333") +
  geom_hline(yintercept = -0.55, size = 1.5, color = "#333333") +
  scale_x_continuous(
    breaks = seq(0, 100, by = 20),
    limits = c(-1.5, 102),
    expand = c(0, 0)) +
...
```{r}
alg_1_pct_prof_adv <- schools %>%
  filter(grade == "All Grades", subgroup == "All Students") %>%
  filter(subject == "Algebra I") %>%
  transmute(valid_tests, pct_prof_adv_above_95 = pct_prof_adv > 95)

t.test(valid_tests ~ pct_prof_adv_above_95, alg_1_pct_prof_adv, alternative = "greater")
```
...
t.test(valid_tests ~ pct_prof_adv_below_25, alg_1_pct_prof_adv, alternative = "greater")
```
